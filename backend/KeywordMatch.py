...
class KeywordMatch():
  ...
  def find_keyword_match(self, text1, text2):
    keywords1 = [x[0] for x in sorted(self.custom_kw_extractor.extract_keywords(text1), key=lambda x: x[1], reverse=True)]
    keywords2 = [x[0] for x in sorted(self.custom_kw_extractor.extract_keywords(text2), key=lambda x: x[1], reverse=True)]
    ...
    return len(set.intersection(keyword_set_1, keyword_set_2)) >= threshold